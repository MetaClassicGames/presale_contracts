...
contract RookieE0 is ERC721, ERC721Enumerable, Pausable, AccessControl, ERC721Burnable, ReentrancyGuard {
    ...
    // URI of the tokens
    string public baseURI = "http://vegabuild.es/mcg_tokens/0.json";
    ...
    // Events
    event RookieMinted(uint256 indexed tokenId, address to);
     ...
     */
    constructor(address selectedStablecoin, uint256 newPrice) ERC721("RookieEd0", "RKE0") {
    ...
    function doPrivateSell() external whenNotPaused nonReentrant {
        ...
        require(_privateSell[msg.sender] > 0, "RKE0: Beneficiary isn't into private sell");
        require(_endOfPS, "RKE0: Presale is not over yet");

        for(uint8 i = 0; i < _privateSell[msg.sender]; i++){
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            emit RookieMinted(tokenId, msg.sender);
