...
async function main() {
  ...
  const rookie = await Rookie.deploy(
    ...
    hre.ethers.utils.parseEther(rookiePrice.toString())
  );
  ...
  const amateur = await Amateur.deploy(
    hre.ethers.constants.AddressZero,
    hre.ethers.utils.parseEther(amateurPrice.toString())
  );
  ...
  const pro = await Pro.deploy(
    hre.ethers.constants.AddressZero,
    hre.ethers.utils.parseEther(proPrice.toString())
  );
  await pro.deployed();
  ...
  const legend = await Legend.deploy(
    hre.ethers.constants.AddressZero,
    hre.ethers.utils.parseEther(legendPrice.toString())
  );
