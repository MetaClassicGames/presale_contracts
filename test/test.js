...
describe("Deploy", function(){
  ...
  this.beforeAll(async function(){
    try{
      ...
      rookieContract = await Rookie.deploy(beneficiaries, quantities, limit, ethers.constants.AddressZero);
...
describe("Presale Contract Test Suite", function () {
  ...
  it("Roles control", async function(){
    // R1
    await expect(rookieContract.connect(admin).setNewStableCoin(usdContract.address),'REVERT: ROLES').to.be.reverted;
    // R4
    await expect(rookieContract.connect(admin).setNewPrice(ethers.utils.parseEther(rookiePrice.toString())),'REVERT: ROLES').to.be.reverted;
    // R6
    await expect(rookieContract.connect(admin).setNewDate(1),'REVERT: ROLES').to.be.reverted;
    // R8
    await expect(rookieContract.connect(admin).setChildContractAddress(admin.address),'REVERT: ROLES').to.be.reverted;
    // R13
    await expect(rookieContract.connect(admin).setURI("hola"),'REVERT: ROLES').to.be.reverted;
    // RX
    await expect(rookieContract.connect(admin).setNewPrivateSell(admin.address, 1),'REVERT: ROLES').to.be.reverted;
    // R25
    await expect(rookieContract.connect(admin).withdrawFunds(admin.address),'REVERT: ROLES').to.be.reverted;
    // R25
    await expect(rookieContract.connect(admin).nativeWithdraw(admin.address),'REVERT: ROLES').to.be.reverted;

    const ADMIN_ROLE = ethers.utils.id("ADMIN_ROLE");
    
    // RolesR2
    await expect(rookieContract.connect(admin).grantRole(ADMIN_ROLE,admin.address),'REVERT: roles').to.be.reverted;
    // RolesV1
    await expect(rookieContract.grantRole(ADMIN_ROLE, admin.address),'ROL ACTIONER is ADMIN').not.to.be.reverted;
  });

  it("setNewStableCoin fx", async function(){
    // R2 
    await expect(rookieContract.connect(admin).setNewStableCoin(ethers.constants.AddressZero),'Zero address for StableCoin').to.be.revertedWith("RKE0: StableCoin address shouldn't be zero address");
    //TODO R3
    // await expect(rookieContract.setNewStableCoin(B1.address),'NO ERC20 address for StableCoin').to.be.reverted;
    // V1
    await expect(rookieContract.connect(admin).setNewStableCoin(usdContract.address),'USDC set').not.to.be.reverted;
  ...
  it("setNewPrice fx", async function(){
    ...
    // R5
    await expect(rookieContract.connect(admin).setNewPrice(0),'REVERT: New price 0').to.be.revertedWith("RKE0: Price must be greater than 0 wei");
    // V2
    await expect(rookieContract.connect(admin).setNewPrice(expected),'New price set').not.to.be.reverted;
  ...
  it("setNewDate fx", async function(){
    ...
    // V3
    await expect(rookieContract.connect(admin).setNewDate(newTS),'New date set').not.to.be.reverted;
  ...
  it("setChildContractAddress fx", async function(){
    // R9
    await expect(rookieContract.connect(admin).setChildContractAddress(ethers.constants.AddressZero),'REVERT: zeroAddress child contract').to.be.revertedWith("RKE0: Zero address shouldn't be child contract");
    ...
    // V4
    await expect(rookieContract.connect(admin).setChildContractAddress(te1.address),'child contract address seted').not.to.be.reverted;
  ...
  it("Mint 1 Rookie Ed 0 for owner", async function(){
    ...
    // R20
    await expect(rookieContract.mintRookie(ethers.constants.AddressZero),'REVERT: Mint Rookie ADDRESS ZERO').to.be.revertedWith("RKE0: Address shouldn't be zero");
    // R21
    await expect(rookieContract.mintRookie(owner.address),'REVERT: Allowance < rookiePrice').to.be.reverted;
    // R22
    await expect(rookieContract.mintRookie(admin.address),'REVERT: Admin has 0 USDC').to.be.revertedWith("USDC: Insufficient funds");

    // Approval
    if(parseInt(await usdContract.allowance(owner.address, rookieContract.address)) < parseInt(ethers.utils.parseEther(rookiePrice.toString()))){
      await expect(usdContract.approve(rookieContract.address, ethers.utils.parseEther(rookiePrice.toString())),'Mint Approve').not.to.be.reverted;
    }

    // Mint
    await expect(rookieContract.mintRookie(owner.address),'Mint Rookie tk0').to.emit(rookieContract, "RookieMinted");
  ...
  it("Should revert when safeMint function is called", async function(){
    // R24 Deployer 
    await expect(rookieContract.safeMint(owner.address),'REVERT: Mint deployer').to.be.reverted;
    // R24 Admin
    await expect(rookieContract.connect(admin).safeMint(admin.address),'REVERT: Mint admin').to.be.reverted;
  });
  ...
  it("Should change the URI", async function(){
    ...
    //V7
    await expect(rookieContract.connect(admin).setURI(newURI),'New URI seted').not.to.be.reverted;
  ...
  it("setNewPrivateSell fx", async function(){
    ...
    // R15
    await expect(rookieContract.setNewPrivateSell(ethers.constants.AddressZero, 1),'REVERT: Private sell to address zero').to.be.revertedWith("RKE0: Address shouldn't be zero");
    // V9
    await expect(rookieContract.connect(admin).setNewPrivateSell(admin.address, 50),'Admin added to private sell').not.to.be.reverted;

    const real_quantity = await rookieContract.getPrivateSell(admin.address);
    const new_privatelycount  = 150;

    //* Success criteria
    expect(new_quantity,'new quantity to admin added to private sell').to.equal(real_quantity);
    expect(await rookieContract.getPrivatelySellCount()).to.equal(new_privatelycount);
  });
  ...
  it("Should do a private sell to B1", async function () {
    ...
    await expect(rookieContract.connect(admin).setNewDate(endOfPS),'Set new PS date').not.to.be.reverted;

    // R17
    await expect(rookieContract.connect(owner).doPrivateSell(),"REVERT: owner isn't into private sell").to.be.revertedWith("RKE0: Beneficiary isn't into private sell");
    ...
    expect(await rookieContract.getPrivateSell(B1.address),'Check if B1 is seted to 0 in mapping after private sell').to.equal(0);
  });
  ...
  it("Should withdraw USDC balance to admin wallet", async function(){
    ...
    // R26
    await expect(rookieContract.connect(admin).withdrawFunds(ethers.constants.AddressZero),'REVERT: Withdraw USDC to address zero').to.be.revertedWith("RKE0: Address shouldn't be zero");

    const adminBalanceBefore = await usdContract.balanceOf(admin.address);
    expect(parseInt(ethers.utils.formatEther(adminBalanceBefore)),'Admin balance before').to.equal(0);

    //V12
    await expect(rookieContract.connect(admin).withdrawFunds(admin.address),'Withdraw USDC').not.to.be.reverted;
    ...
    expect(parseInt(ethers.utils.formatEther(adminBalanceAfter)),'Admin balance before').to.equal(rookiePrice);
    expect(parseInt(ethers.utils.formatEther(contractBalance)),'Admin balance before').to.equal(0);
  });
  ...
  it("Should revert when B1 address try to call setCounterBreeding", async function(){
    ...
    await expect(rookieContract.setCounterBreeding(2, B1.address),'REVERT: setCounterBreeding').to.be.revertedWith("RKE0: Caller isnt the child contract")
  });

  it("Reproduce 2 and 3", async function(){
    expect(await rookieContract.getCounterBreeding(2),'getCounterBreeding').to.equal(0);
    expect(await rookieContract.getCounterBreeding(3),'getCounterBreeding').to.equal(0);

    await expect(te1.connect(B1).reproduce(2,3),'Breed').to.emit(te1, "MintedByBreed");

    expect(await rookieContract.getCounterBreeding(2),'getCounterBreedingAfterReproduce1').to.equal(1);
    expect(await rookieContract.getCounterBreeding(3),'getCounterBreedingAfterReproduce1').to.equal(1);
  });
  ...
  it("Should revert when 2 and 3 calls reproduce when it calls again", async function(){
    ...
    expect(await rookieContract.getCounterBreeding(3),'getCounterBreedingAfterReproduce1').to.equal(2);

    await expect(te1.connect(B1).reproduce(2,3),'REVERT: BREED').to.be.revertedWith("MTK1: Token 1 is steril");
  });
